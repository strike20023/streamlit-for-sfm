...
class DecisionTreeNode:
    ...
    def to_dot(self, graph=[], parent_id=None, edge_label=""):
        node_id = len(graph)
        # 生成当前节点的标签
        labels = [f"{cls}: {count}" for cls, count in zip(["True", "False"], self.num_samples_per_class)]
        # feature_ranges
        ...
        if parent_id is not None:
            # 为当前节点和父节点之间添加边
            graph.append(f"n{parent_id} -> n{node_id} [label=\"{edge_label}\"]")
        
        # 递归为子节点生成DOT描述
        if self.left:
            self.left.to_dot(graph, node_id, f"{self.feature_index} <= {self.threshold}")
        if self.right:
            self.right.to_dot(graph, node_id, f"{self.feature_index} > {self.threshold}")
        return graph
...
def split(dataset, feature_index, threshold):
    """根据特征索引和阈值分割数据集"""
    left = [sample for sample in dataset if sample[0][feature_index] <= threshold]
    right = [sample for sample in dataset if sample[0][feature_index] > threshold]
    return left, right

def best_split(dataset, feature_indexs):
    """寻找最佳分割"""
    best_index, best_threshold, best_gini, best_splits = None, None, float('inf'), None
    for feature_index in feature_indexs:  # 遍历每个特征
        ...
        for threshold in thresholds:
            ...
            if gini < best_gini:
                best_gini, best_index, best_threshold, best_splits = gini, feature_index, threshold, (left, right)
...
def build_tree(dataset, num_selected_features, depth=0, max_depth=1000, feature_ranges=None):
    ...
    if feature_ranges is None:
        feature_ranges = [(float('inf'), float('-inf'))] * len(num_selected_features)
    ...
    if not all(num_samples_per_class):
        return node
    feature_index = [idx for idx,i in enumerate(num_selected_features) if i==1]
    index, threshold, splits = best_split(dataset, feature_index)
    if splits:
        pass  
    elif all(num_samples_per_class):
        num_selected_features = pd.Series(num_selected_features).shift(1).fillna(0).to_list()
        # num_selected_features = [0 if i==max(num_selected_features) else i for i in range(len(num_selected_features))]
        feature_index = [idx for idx,i in enumerate(num_selected_features) if i==1]
        index, threshold, splits = best_split(dataset, feature_index)
    else:
        return node
    ...
    node.left = build_tree(left, num_selected_features, depth + 1, max_depth,l_feature_ranges)
    node.right = build_tree(right, num_selected_features, depth + 1, max_depth,r_feature_ranges)
    return node
...
def print_leaf_nodes(node):
    ...
    if node is not None:
        if node.left is None and node.right is None:  # 叶子节点
            # print(node)
            leaf_info.append((node.feature_ranges, node.predicted_class))
        else:
...
标签 = ~数据集["done"]
dataset = [(特征.to_numpy()[i].tolist(), 标签.to_numpy()[i]) for i in range(len(特征))]
# pprint(dataset)
num_selected_features=[0 for i in range(len(dataset[0][0]))]
num_selected_features[0]=1
tree = build_tree(dataset, num_selected_features=num_selected_features)
dot_representation = print_dot(tree)
print(dot_representation)
结果表 = print_leaf_nodes(tree)
结果表 = [[*[k for t in i[0] for k in t],i[1]] for i in 结果表]
结果表 = pd.DataFrame(结果表, columns=[*[i+t for i in 特征.columns for t in ['_l','_r']], "接受"])
结果表 = 结果表[结果表["接受"] == True]
# %%
